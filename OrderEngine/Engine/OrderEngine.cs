namespace L6R.OrderEngine
{
    ...
    /// Use OrderEngineFactory.Create(BuildMode) to instantiate the appropriate implementation.
    /// </summary>
    ...
    /// 
    /// Usage:
    /// - IOrderEngine engine = OrderEngineFactory.Create(BuildMode.MultipleBuild);  // For multiple Build() calls
    /// - IOrderEngine engine = OrderEngineFactory.Create(BuildMode.SingleBuild);    // For single Build() call
    /// 
    /// Both implementations share the same interface, so tests and client code are identical.
    /// </summary>
    public static class OrderEngineFactory
    {
        /// <summary>
